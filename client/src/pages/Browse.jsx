...
import { Search, MapPin, Clock, Filter, X, Heart, Sparkles } from 'lucide-react';
...
const Browse = () => {
  ...
  const savedKey = `nexuslearn-saved-${currentUser?._id || 'guest'}`;
  const [savedIds, setSavedIds] = useState(() => {
    try { return JSON.parse(localStorage.getItem(`nexuslearn-saved-${currentUser?._id || 'guest'}`) || '[]'); } catch { return []; }
  });
  useEffect(() => {
    const skill = searchParams.get('skill');
    if (skill) { setSearchTerm(skill); setFilters((current) => ({ ...current, skill })); }
  }, [searchParams]);
  useEffect(() => {
  ...
  const isSkillMatch = (person) => {
    const names = (items = []) => items.map((item) => item.name?.trim().toLowerCase()).filter(Boolean);
    const personOffers = names(person.skillsOffered);
    const personWants = names(person.skillsWanted);
    const myOffers = names(currentUser?.skillsOffered);
    const myWants = names(currentUser?.skillsWanted);
    return personOffers.some((skill) => myWants.includes(skill)) || personWants.some((skill) => myOffers.includes(skill));
  };
  const publicUsers = users.filter(user => user.isPublic && user._id !== currentUser?._id);
  const visibleUsers = publicUsers.filter((person) => view === 'saved' ? savedIds.includes(person._id) : view === 'matches' ? isSkillMatch(person) : true);
  const totalPages = Math.ceil(visibleUsers.length / USERS_PER_PAGE);
  const paginatedUsers = visibleUsers.slice((currentPage - 1) * USERS_PER_PAGE, currentPage * USERS_PER_PAGE);
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      console.error('Error fetching users:', (error instanceof Error ? error.name : "UnknownError"));
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-white pt-8 px-2 animate-fade-in">
      <div className="w-full max-w-6xl flex flex-col items-start mb-8 mt-4">
        ...
        <div className="mt-5 flex flex-wrap gap-2" role="tablist" aria-label="Member discovery views">
          {[['all', 'Everyone'], ['matches', 'Skill matches'], ['saved', 'Saved people']].map(([key, label]) => <button key={key} role="tab" aria-selected={view === key} onClick={() => { setView(key); setCurrentPage(1); }} className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold transition ${view === key ? 'border-brand-plum bg-brand-plum text-white' : 'border-brand-plum/20 bg-white text-brand-plum hover:bg-brand-plum/5'}`}>{key === 'matches' && <Sparkles className="h-4 w-4" />}{key === 'saved' && <Heart className="h-4 w-4" />}{label}</button>)}
        </div>
      </div>
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col gap-8">
        {visibleUsers.length === 0 ? (
          <div className="text-center py-12 animate-fade-in" style={{ animationDelay: '0.3s' }}>
            <p className="text-brand-orchid text-lg">{view === 'saved' ? 'No saved people yet. Use the heart on a profile to keep them close.' : view === 'matches' ? 'No skill matches yet. Add skills you offer and want on your profile to find better matches.' : 'No users found'}</p>
          </div>
        ) : (
          paginatedUsers.map((user, idx) => (
            ...
            >
              ...
              <div className="flex flex-col items-end justify-between h-full min-w-[160px] gap-4">
                <button type="button" onClick={() => toggleSaved(user._id)} aria-label={savedIds.includes(user._id) ? `Remove ${user.name} from saved people` : `Save ${user.name}`} aria-pressed={savedIds.includes(user._id)} className={`rounded-full border p-2 transition ${savedIds.includes(user._id) ? 'border-rose-200 bg-rose-50 text-rose-600' : 'border-gray-200 text-gray-500 hover:border-rose-200 hover:text-rose-500'}`}><Heart className={`h-5 w-5 ${savedIds.includes(user._id) ? 'fill-current' : ''}`} /></button>
