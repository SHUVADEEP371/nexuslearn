...
const SwapDetail = () => {
  ...
  const [sessionMode, setSessionMode] = useState('CREDIT');
  ...
  const handleScheduleSession = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || 'Could not schedule this session');
    }
  };
    ...
    <div className="min-h-screen flex flex-col items-center bg-white pt-8 px-2 animate-fade-in">
        ...
        {swap.status === 'accepted' && (
          <>
            <button onClick={() => setShowScheduleModal(true)} className="btn btn-primary">
              <Video className="w-4 h-4 mr-2" /> Schedule a learning session
            </button>
            <button onClick={handleComplete} className="btn btn-secondary">
              <Check className="w-4 h-4 mr-2" /> Mark exchange as completed
            </button>
          </>
        )}
      ...
      {showScheduleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <form onSubmit={(event) => { event.preventDefault(); void handleScheduleSession(); }} className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <h2 className="text-xl font-bold text-gray-900">Plan your learning session</h2>
            <p className="mt-1 text-sm text-gray-600">Choose a time in your local timezone. NexusLearn stores the session time as UTC.</p>
            <label className="mt-5 block text-sm font-semibold text-gray-700">Start time</label>
            <input type="datetime-local" required min={new Date(Date.now() + 5 * 60_000 - new Date().getTimezoneOffset() * 60_000).toISOString().slice(0, 16)} value={scheduleStart} onChange={(event) => setScheduleStart(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 p-3" />
            <label className="mt-4 block text-sm font-semibold text-gray-700">Duration</label>
            <select value={sessionDuration} onChange={(event) => setSessionDuration(Number(event.target.value))} className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-3"><option value={30}>30 minutes</option><option value={60}>60 minutes</option><option value={90}>90 minutes</option><option value={120}>2 hours</option></select>
            <label className="mt-4 block text-sm font-semibold text-gray-700">Payment method</label>
            <select value={sessionMode} onChange={(event) => setSessionMode(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-3"><option value="CREDIT">Use 1 Skill Credit</option><option value="PAID">Pay with UPI / Razorpay</option></select>
            {sessionMode === 'PAID' && <p className="mt-2 text-xs text-gray-500">The platform price is calculated from the server's configured hourly rate and shown by checkout.</p>}
            <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setShowScheduleModal(false)} className="rounded-lg border border-gray-300 px-4 py-2 font-semibold text-gray-700">Cancel</button><button className="rounded-lg bg-brand-plum px-4 py-2 font-semibold text-white">Create session</button></div>
          </form>
        </div>
      )}
